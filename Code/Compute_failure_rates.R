...
# minus the per-recurrence product of reinfection; see
# Compare_failure_recur_independence.R for how much this differs to using
# all-reinfection sequence probabilities. Estimates based on time-to-event only
# are used where genetically informed estimates are not available and for
# censored observations. The code is based on Pooled_Analysis.Rmd but it differs
# (see notes below).
# 
...
# computation; use of normalised ones makes negligible difference: 
...
Combined_Time_Data$Pv3Rs <- NA # To record if at least one recurrence with genetic-based probability
Combined_Time_Data$Pv3Rs_joint <- NA # To record if genetic-based probability generated jointly
...
# Check prototype median estimates agree over old data sets where overlap
...
Results <- data.frame(Pv3Rs = TimeToEvent_Pv3Rs[,"I"], 
                      Pv3Rs_joint = TimeToEvent_Pv3Rs[,"joint"], 
                      prototype_median[rownames(TimeToEvent_Pv3Rs)], # NA for VHX_583_2
                      prototype_mean[rownames(TimeToEvent_Pv3Rs)])   # NA for VHX_583_2
...
for(i in 1:nrow(Combined_Time_Data)){ 
  ...
  if (eid %in% rownames(Results)) { # Use genetic episode if available
    ...
    Combined_Time_Data$Pv3Rs[i] <- TRUE
    Combined_Time_Data$Pv3Rs_joint[i] <- Results$Pv3Rs_joint[j]
    ...
    Combined_Time_Data$Pv3Rs[i] <- FALSE
  }
}

# Special case: among estimates generated by Pv3Rs but not the prototype, only
# one recurrence (VHX_583_2) was in a PMQ treated individual; genetic data on
# VHX_583_3 was analysed by the prototype
i <- which(Combined_Time_Data$episodeid == "VHX_583_2")
j <- which(rownames(prior) == "VHX_583_2")
Combined_Time_Data$I_prototype_median[i] <- prior_unnorm$I[j]
Combined_Time_Data$I_prototype_mean[i] <- prior$I[j]
...
patient_summaries = Combined_Time_Data[!duplicated(Combined_Time_Data$patientid),
                                       c("patientid", "arm_num", "FU_time", "Pv3Rs", "Pv3Rs_joint")]
...
for(i in 1:nrow(patient_summaries)){
  ...
  patient_summaries$Pv3Rs[i] <- any(Combined_Time_Data[ind, "Pv3Rs"], na.rm = T)
  
  # Use mean not all because bug-causing all(c(NA, NA), na.rm = TRUE) = TRUE:
  patient_summaries$Pv3Rs_joint[i] <- mean(Combined_Time_Data[ind, "Pv3Rs_joint"]*1, na.rm = T) 
}

# ==============================================================================
# Recompute study-level failure rate estimates, checking prototype_median agrees
# with Pooled_Analysis (internal control). Old results: based on median
# reinfection probability (see lines 1791 and 1792). Specifically, from
# Pooled_Analysis.Rmd,
# 3.047353 for BPD, line 2008,
# 2.368886 for VHX, line 2018, 
# 2.889866 for all, line 2038. 
# ==============================================================================
...
ignore_col <- which(colnames(patient_summaries) == "Pv3Rs_joint")
if (any(is.na(patient_summaries[, -ignore_col]))) stop("NA failures")
...
# Un-rounded for checking with Pooled_Analysis.Rmd,
cbind(BPD, VHX, All)[1,]
# 3.047353 for BPD, line 2008,
# 2.368886 for VHX, line 2018, 
# 2.889866 for all, line 2038. 

# Rounded for copy paste to overleaf:
round(cbind(BPD, VHX, All), 2)
# Number PQ-treated:
sum(ind_all) 
# Number PQ-treated with at least one Pv3Rs probability:
sum(patient_summaries$Pv3Rs[ind_all]) 
# Number PQ-treated with at least one Pv3Rs probability generated jointly:
sum(patient_summaries$Pv3Rs_joint[ind_all], na.rm = T)   
...
prototype_BPD_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_prototype_median[ind_BPD])/sum(ind_BPD)
prototype_VHX_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_prototype_median[ind_VHX])/sum(ind_VHX)
prototype_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_prototype_median[ind_all])/sum(ind_all)

Pv3Rs_BPD_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_Pv3Rs[ind_BPD])/sum(ind_BPD)
Pv3Rs_VHX_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_Pv3Rs[ind_VHX])/sum(ind_VHX)
Pv3Rs_CI <- 100*qpbinom(c(0.025,0.975), probs = patient_summaries$Failure_Pv3Rs[ind_all])/sum(ind_all)

prototype_CIs <- c(sprintf("(%s)", paste(round(prototype_BPD_CI, 2), collapse = " ")), 
                   sprintf("(%s)", paste(round(prototype_VHX_CI, 2), collapse = " ")),
                   sprintf("(%s)", paste(round(prototype_CI, 2), collapse = " ")))

Pv3Rs_CIs <- c(sprintf("(%s)", paste(round(Pv3Rs_BPD_CI, 2), collapse = " ")), 
               sprintf("(%s)", paste(round(Pv3Rs_VHX_CI, 2), collapse = " ")),
               sprintf("(%s)", paste(round(Pv3Rs_CI, 2), collapse = " ")))

# Copy and paste to overleaf: 
prototype_CIs 
...
legend("left", pch = 16:17, bty = "n",
       legend = c("BPD study participant", 
                  "VHX study participant"), )
