################################################################################
# Script first written in 2023 to summarise which patients and recurrences are
# missing estimates in Taylor&Watson et al. 2019. There are three issues around
...
# recurrences were omitted due to computational complexity")
#
# 1) Estimation impossible: data on only one episode
# 2) Estimation impossible: 6 or more genotypes in 2 episodes
# 3) Data for which estimation was slow: UpperComplexityIDs
# 
# In this script I use MS_final generated by Pool_Analysus.Rmd. I think
...
# example, is just left over from line 1985). Results for directly estimated
# only in:
#
...
# Set up
setwd("/Users/ataylor/Documents/RecurrentVivax/Pooled_Final_Analysis")
if (!exists("MS_final")) stop("Generate MS_final by running chunks of Pool_Analysis.Rmd")
list_all <- ls()
log_MS_final <- grepl("MS_final", list_all)
rm(list = list_all[!log_MS_final]) # Remove all but MS_final (and missing_from_MS_final)
...
# Load resources to plot data (these could be now replaced by Pv3Rs plotting function)
source("~/Documents/RecurrentVivax/Genetic_Model/PlottingFunction.R")
...
#
# "VHX_239" "VHX_461" "VHX_52"  # These overlap with missingIDs
...
# assuming we actually meant take a complement if P(L)+P(C) <= 1 OR re-weight.
